...
public class Solution {
    ...
    public int[] findRightInterval(int[][] intervals) {
        ...
        int[] ans = new int[n];
        Map<Integer, Integer> map = new HashMap<>();//start和index的映射
        ...
        for (int i = 0; i < n; i++) {
            int[] target = find(start, intervals[i][1]);
            if (target[0] == -1) {
                ans[i] = -1;
            } else {
                ans[i] = map.get(target[1]);
            }
        }
        return ans;
    }

    private int[] find(List<Integer> list, int bound) {
        if (bound == -1) {
